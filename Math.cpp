...
Vector2 Vector2::Transform( const Vector2& vector, const Matrix3& matrix, float w /*= 1.0f*/ ) noexcept
{
	Vector2 retVal;
	retVal.x = vector.x * matrix.matrix[0][0] + vector.y * matrix.matrix[1][0] + w * matrix.matrix[2][0];
	retVal.y = vector.x * matrix.matrix[0][1] + vector.y * matrix.matrix[1][1] + w * matrix.matrix[2][1];
	
	return retVal;
}

Vector3 Vector3::Transform( const Vector3& vector, const Matrix4& mat, float w /*= 1.0f*/ ) noexcept
{
	Vector3 retVal;

	retVal.x = vector.x * mat.mat[0][0] + vector.y * mat.mat[1][0] +
		vector.z * mat.mat[2][0] + w * mat.mat[3][0];
	retVal.y = vector.x * mat.mat[0][1] + vector.y * mat.mat[1][1] +
		vector.z * mat.mat[2][1] + w * mat.mat[3][1];
	retVal.z = vector.x * mat.mat[0][2] + vector.y * mat.mat[1][2] +
		vector.z * mat.mat[2][2] + w * mat.mat[3][2];
	
	return retVal;
}

Vector3 Vector3::TransformWithPerspDiv(const Vector3& vec, const Matrix4& mat, float w /*= 1.0f*/) noexcept
{
	Vector3 retVal;
	...
	if ( false == Math::NearZero( Math::Abs( transformedW ) ) )
	{
...
Vector3 Vector3::Transform( const Vector3& vector, const Quaternion& quaternion) noexcept
{
	Vector3 qv( quaternion.x, quaternion.y, quaternion.z );
	Vector3 retVal = vector;
	
	retVal += 2.0f * Vector3::Cross( qv, Vector3::Cross( qv, vector ) + quaternion.w * vector );

	return retVal;
}

void Matrix4::Invert() noexcept
{
	float tmp[12];
	float src[16];
	float dst[16];
...
Matrix4 Matrix4::CreateFromQuaternion( const class Quaternion& q ) noexcept
{
	float mat[4][4];
